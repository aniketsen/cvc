...
#trace ( loop ) * prop
do_seq_ss <- function(loop,prop) {
  trloop <- array(0,dim=vol)
  for (x in 1:vol) {
    for (i in 1:12) {
      trloop[x] <- trloop[x] + loop[i,x,i]
    }
  }
  ...
  for (x in 1:vol) {
    for (i in 1:12) {
      for(k in 1:12) {
        seq_source[i,x,k] <- -trloop[x]*prop[k,x,i]
      }
    }
  }
  return(seq_source)
...
#trace ( gamma_mu * loop ) * gamma_mu * prop
do_seq_vv <- function(loop,prop) {
  trloop <- array(0,dim=c(4,vol))
  for (mu in 1:4) {
    for (x in 1:vol) {
      for (alpha in 1:4) {
        for (a in 1:3) {
          ...
          for (beta in 1:4) {
            kb <- (beta-1)*3+a
            trloop[mu,x] <- trloop[mu,x] + gam[mu,alpha,beta]*loop[kb,x,ka]
          }
        }
      }
    }
      ...
      for (alpha in 1:4) {
        for (a in 1:3) {
          ...
          for (beta in 1:4) {
            kb <- (beta-1)*3+a
            for (kc in 1:12) {
              seq_source[ka,x,kc] <- seq_source[ka,x,kc] - trloop[mu,x]*gam[mu,alpha,beta]*prop[kc,x,kb]
            }
          }
        }
      }
...
#trace ( gamma_mu * gamma_5 * loop ) * gamma_mu * gamma_5 * prop
do_seq_aa <- function(loop,prop) {
  trloop <- array(0,dim=c(4,vol))
  for (mu in 1:4) {
    for (x in 1:vol) {
      for (alpha in 1:4) {
        for (a in 1:3) {
          ...
          for (beta in 1:4) {
            kb <- (beta-1)*3+a
            trloop[mu,x] <- trloop[mu,x] + gam5mu[mu,alpha,beta]*loop[kb,x,ka]
          }
        }
      }
    }
      ...
      for (alpha in 1:4) {
        for (a in 1:3) {
          ...
          for (beta in 1:4) {
            kb <- (beta-1)*3+a
            for (kc in 1:12) {
              seq_source[ka,x,kc] <- seq_source[ka,x,kc] - trloop[mu,x]*gam5mu[mu,alpha,beta]*prop[kc,x,kb]
            }
          }
        }
      }
...
#trace ( gamma_5 * loop ) * gamma_5 * prop
do_seq_pp <- function(loop,prop) {
  trloop <- array(0,dim=vol)
    for (x in 1:vol) {
      for (alpha in 1:4) {
        for (a in 1:3) {
          ...
          for (beta in 1:4) {
            kb <- (beta-1)*3+a
            trloop[x] <- trloop[x] + gam5[alpha,beta]*loop[kb,x,ka]
          }
        }
      }
    }
      ...
      for (alpha in 1:4) {
        for (a in 1:3) {
          ...
          for (beta in 1:4) {
            kb <- (beta-1)*3+a
            for (kc in 1:12) {
              seq_source[ka,x,kc] <- seq_source[ka,x,kc] - trloop[x]*gam5[alpha,beta]*prop[kc,x,kb]
            }
          }
        }
      }
